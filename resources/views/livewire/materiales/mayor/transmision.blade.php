<div>
    ...
    @canany([
        ...
        'Material Mayor Transmision
        Eliminar',
        ])
        <x-card-form>
            <x-card-input label="Transmisión" placeholder="Transmisión..." campo="transmision" :disabled="in_array($modo, ['inicio', 'seleccionado'])" />


            <x-slot name="buttons">
                @can('Material Mayor Transmision Crear')
                ...
                @can('Material Mayor Transmision Editar')
                ...
                @can('Material Mayor Transmision Eliminar')
                ...
                @canany(['Material Mayor Transmision Crear', 'Material Mayor Transmision Editar'])
    ...
    <!-- Tabla -->
    <x-tabla titulo="Parametros - Transmisión">
        ...
        @foreach ($transmisiones as $transmision)
            <tr wire:click="seleccionado({{ $transmision->id_movil_transmision }})"
                wire:key="{{ $transmision->id_movil_transmision }}">
                <td>{{ $transmision->transmision ?? 'N/A' }}</td>
                <td>{{ $transmision->activo ? 'Activo' : 'Inactivo' }}</td>
                <td>
                    <button class="btn btn-sm" style="border:none;background:transparent;"
                        wire:click="cambiarEstado({{ $transmision->id_movil_transmision }}, true)"
                    ...
                    <button class="btn btn-sm" style="border:none;background:transparent;"
                        wire:click="cambiarEstado({{ $transmision->id_movil_transmision }}, false)"
